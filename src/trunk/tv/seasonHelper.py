...
class SeasonHelper:
  ...
  @staticmethod
  def setKeyForFilename(epMap, newKey, filename):
    utils.verifyType(epMap, episode.EpisodeMap)
    ...
    sourceEp = None
    for key in ret.matches_.keys():
      if epMap.matches_[key].filename_ == filename:
        sourceEp = epMap.matches_[key]
        break
    if not sourceEp:
      for ep in ret.unresolved_:
    ...
    if oldEpNum == episode.UNRESOLVED_KEY:
      ...
      ret.unresolved_.remove(sourceEp)
      if ret.matches_.has_key(newKey):
        oldEp = copy.copy(ret.matches_[newKey])
        oldEp.epNum_ = episode.UNRESOLVED_KEY
        ret.unresolved_.append(oldEp)
      ret.matches_[sourceEp.epNum_] = sourceEp
    else: #oldEpNum in matches
      del ret.matches_[oldEpNum]
      if newKey == episode.UNRESOLVED_KEY:
        ret.unresolved_.append(sourceEp)
